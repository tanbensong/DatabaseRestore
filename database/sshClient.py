...
class SSHAgent(object):
    """远程还原"""
    def __init__(self, host, port=1022, user="feidao", password="fdcf@2020", pkeyfile=''):
        """初始化
        ...
        if pkeyfile:
            self.transport.connect(
                username=user,
                pkey=RSAKey.from_private_key_file(pkeyfile)
            )
        else:
            self.transport.connect(
                username=user,
    ...
    def findPath(self, path):
        ...
        try:
            ...
            for dateStr in timelists:
                backtime = datetime.strptime(dateStr, "%Y%m%d%H%M%S").strftime("%Y年%m月%d日 %H:%M:%S")
